...
/// Inject kallsyms.bin into the .kallsyms ELF section in-place.
///
/// The kernel binary contains a fixed-size placeholder array in the
/// `.kallsyms` section. This function overwrites that placeholder with
/// the actual compressed symbol table data generated by `generate_symbols`.
///
/// Uses in-place write (same inode) to preserve hardlinks created by cargo.
/// `rust-objcopy --update-section` creates a new file (new inode), which
/// breaks cargo's hardlink mechanism and causes cargo to restore the old
/// (un-injected) ELF on the next build.
///
/// # Errors
///
/// Returns an error if:
/// - `kallsyms.bin` exceeds `KALLSYMS_RESERVE_SIZE` (512 KB)
/// - The `.kallsyms` section cannot be found in the ELF
pub fn inject_kallsyms(elf_path: &Path, kallsyms_path: &Path) -> Result<()> {
    let kallsyms_data = fs::read(kallsyms_path)
        .with_context(|| format!("Failed to read {}", kallsyms_path.display()))?;

    if kallsyms_data.len() > KALLSYMS_RESERVE_SIZE {
        anyhow::bail!(
    ...
    // Parse the ELF to find the .kallsyms section offset and size
    ...
    if kallsyms_data.len() > section_size {
        anyhow::bail!(
            "kallsyms.bin ({} bytes) exceeds .kallsyms section size ({} bytes)",
            kallsyms_data.len(),
            section_size
        );
    }

    // Write in-place: open for writing, seek to section offset, write data + zero padding.
    // This preserves the inode and cargo's hardlinks.
    let mut file = OpenOptions::new()
        .write(true)
        .open(elf_path)
        .with_context(|| format!("Failed to open ELF for writing: {}", elf_path.display()))?;

    file.seek(SeekFrom::Start(section_offset))
        .context("Failed to seek to .kallsyms section")?;
    file.write_all(&kallsyms_data)
        .context("Failed to write kallsyms data")?;

    // Zero-fill the remaining section space
    let remaining = section_size - kallsyms_data.len();
    if remaining > 0 {
        file.write_all(&vec![0u8; remaining])
            .context("Failed to zero-fill remaining section space")?;
    }

    file.flush().context("Failed to flush ELF file")?;

    println!(
        "Injected kallsyms ({} bytes, {:.1}% of {} section) in-place into {}",
        kallsyms_data.len(),
        (kallsyms_data.len() as f64 / section_size as f64) * 100.0,
        section_size,
        elf_path.display()
    );
    Ok(())
